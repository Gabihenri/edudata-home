...
export type CurriculumResolverRequest = {
  ...
  applicabilityRules: CurriculumApplicabilityRule[]
  input: CurriculumResolutionInput
}

export type CurriculumResolverState = {
  result: CurriculumResolutionResult | null
  loading: boolean
  error: string | null
  resolve: (
    request: CurriculumResolverRequest,
...
const curriculumResolutionCache =
  new Map<string, CurriculumResolutionResult>()
...
function createCacheKey(
  ...
  return JSON.stringify({
    frameworkIds: request.frameworks
      .map(framework => framework.id)
      .sort(),

    versionIds: request.versions
      .map(version => version.id)
      .sort(),

    applicabilityRuleIds: request.applicabilityRules
      .map(rule => rule.id)
      .sort(),
    ...
    context: {
      ...
      curriculumMatrixId: context.curriculumMatrixId,
    ...
    requestedFrameworkIds:
      normalizeStringArray(
        request.input.requestedFrameworkIds,
      ),

    requestedVersionIds:
      normalizeStringArray(
        request.input.requestedVersionIds,
      ),

    includeInherited:
      request.input.includeInherited,

    includeOptional:
      request.input.includeOptional,
  })
}
...
async function readResponse(
  ...
  try {
    return await response.json() as CurriculumApiResponse
  } catch {
    return {
      success: false,
      resolvedFrameworks: [],
...
function getResponseError(
  ...
  const firstError =
    normalizeMessages(result.errors)[0]
...
function validateRequest(
  ...
  if (request.applicabilityRules.length === 0) {
...
export function useCurriculumResolver():
  ...
  const [
    result,
    setResult,
  ] = useState<CurriculumResolutionResult | null>(
    null,
  )
  ...
  const resolve = useCallback(
    async (
      ...
      const cachedResult =
        curriculumResolutionCache.get(cacheKey)
