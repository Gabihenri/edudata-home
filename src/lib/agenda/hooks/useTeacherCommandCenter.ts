...
function normalizeErrorMessage(error: unknown): string { return error instanceof Error && error.message.trim() ? error.message.trim() : 'Não foi possível gerar o Centro de Comando Docente.' }
...
export function useTeacherCommandCenter(options: UseTeacherCommandCenterOptions = {}): UseTeacherCommandCenterResult {
  ...
  const agendaState = useAgendaIntelligence({ autoLoad })
  ...
  const mountedRef = useRef(true); const abortControllerRef = useRef<AbortController | null>(null); const requestSequenceRef = useRef(0); const processedIntelligenceRef = useRef<string | null>(null)
  ...
  const executeOrchestration = useCallback(async (intelligence: AgendaIntelligenceData): Promise<TeacherPerformanceSnapshot | null> => {
    ...
    if (mountedRef.current) { setError(null); setCurrentStep(null); setStatus('loading_snapshot'); setRefreshing(hasExistingSnapshot) }
    try {
      ...
      const result = await orchestrateTeacherIntelligence({ dashboard_intelligence: intelligence, lessons: mapLessonsToCalendarActivities(operationalResponse.data.lessons), events: [], tasks: operationalResponse.data.tasks, planning_history: operationalResponse.data.planning, snapshot_history: [], period: buildCurrentWeekPeriod(), reference_datetime: buildReferenceDateTime(), teacher_context: teacher, role, maximum_priorities: maximumPriorities }, { signal: controller.signal, onStep: step => { if (mountedRef.current && !controller.signal.aborted && requestSequence === requestSequenceRef.current) setCurrentStep(step) } })
      if (!mountedRef.current || controller.signal.aborted || requestSequence !== requestSequenceRef.current) return null
