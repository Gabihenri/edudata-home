...
type OperationalCollectionError = {
  message: string
}

type OperationalCollectionResult<T> = {
  data:
    T[] |
    null

  error:
    OperationalCollectionError |
    null
}
...
function normalizeUserId(
  ...
  if (
    ...
    throw new Error(
      'O usuário é obrigatório para carregar o ciclo operacional.',
    )
  }
...
export async function loadAgendaOperationalSnapshot({
  client,
  userId,
}: {
  client:
  ...
  AgendaOperationalSnapshotResult
> {
  const normalizedUserId =
    normalizeUserId(
      userId,
