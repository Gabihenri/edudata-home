...
class AnalyticsHumanDecisionsRepository {
  ...
  async create(
    ...
    decidedBy: string,
  ): Promise<AnalyticsHumanDecisionRecord> {
    const { data, error } = await this.client
      .from('analytics_human_decisions')
      .insert({
    ...
    if (error) {
      throw new Error(`Erro ao registrar decisão analítica: ${error.message}`)
    }
  ...
  async findBySignalId(signalId: string): Promise<AnalyticsHumanDecisionRecord[]> {
    ...
    if (error) {
      throw new Error(`Erro ao consultar histórico analítico: ${error.message}`)
    }
