            ...
            return (
              <Link
                ...
                className={[
                  ...
                  active ? 'text-[#071827]' : 'text-slate-500 hover:bg-slate-50 hover:text-[#075F78]',
                ].join(' ')}
              >
                {active ? <span aria-hidden="true" className="absolute inset-x-3 top-0 h-0.5 rounded-full bg-[#0B7491]" /> : null}
                <span className="font-mono text-[9px] font-bold tracking-[0.1em] text-[#0B7491]">{item.code}</span>
                <span className="max-w-full truncate text-[11px] font-bold leading-4 sm:text-xs">{item.label}</span>
              </Link>
            )
          })}
                    ...
                    <div className="mt-2 grid gap-2 sm:grid-cols-2">
                      {group.items.map(item => {
                        ...
                        return (
                          ...
                          >
                            <span className={['shrink-0 font-mono text-[10px] font-bold', active ? 'text-cyan-300' : 'text-[#0B7491]'].join(' ')}>
                              {item.code}
                            </span>
                            <span className="min-w-0">
                              <span className="block font-bold">{item.label}</span>
                              <span className={['mt-1 block text-xs leading-5', active ? 'text-slate-300' : 'text-slate-500'].join(' ')}>
                                {item.description}
                              </span>
                            </span>
                          </Link>
