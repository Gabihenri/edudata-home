...
function getNextStep(dimensions: ScoreDimension[]): string {
  ...
  if (!lowest) {
    return 'Continue acompanhando os registros do ciclo operacional.'
  }

  if (clamp(lowest.score) >= 80) {
    return 'Mantenha o acompanhamento do ciclo e preserve os registros atualizados.'
  }
...
export function TeacherScoreExplanationPanel({
  ...
  return (
    <section
      aria-label="Explicação do E-Score"
    ...
    >
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="rounded-2xl border border-[#0B7491]/20 bg-cyan-50 px-5 py-4">
          ...
          <p className="mt-1 text-3xl font-bold tracking-tight text-[#071827]">
            {clamp(overallScore)}%
          </p>
        </div>
      </div>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <article className="rounded-2xl border border-red-200 bg-red-50 p-4">
          <p className="text-xs font-bold uppercase tracking-[0.14em] text-red-700">
            O que está reduzindo o score
          </p>
          {critical.length === 0 ? (
            <p className="mt-3 text-sm leading-6 text-red-800">
              Nenhuma dimensão crítica foi identificada neste momento.
            </p>
          ) : (
              ...
              {critical.map(item => (
                <li key={item.label} className="rounded-xl border border-red-100 bg-white p-3">
                  ...
                  <p className="mt-1 text-sm leading-5 text-slate-600">
                    {item.description} {getImpactLabel(item.score)}
                  </p>
                </li>
              ))}
            </ul>
        ...
        <article className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
          ...
          {positive.length === 0 ? (
            <p className="mt-3 text-sm leading-6 text-emerald-800">
              Ainda não há uma dimensão com contribuição operacional forte.
            </p>
          ) : (
            <ul className="mt-3 space-y-3">
              {positive.map(item => (
      ...
      <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        <p className="mt-1 text-sm leading-6 text-slate-600">
          {getNextStep(dimensions)} As mudanças devem refletir no próximo processamento do snapshot, mantendo rastreabilidade e governança.
        </p>
      </div>
    </section>
  )
