...
type DataRow = Record<string, number | string | undefined> & { period: string }
...
export default function StatisticalExplorationPanel() {
  ...
  const fullData = useMemo<DataRow[]>(() => {
    ...
    data.observations.filter(observation => !observation.excluded && observation.numericValue !== null).forEach((observation, index) => {
      const key = observation.observedAt ?? observation.recordedAt ?? observation.id
      const variableKey = variableById.get(observation.variableId)
      if (!variableKey) return
      const current = grouped.get(key) ?? { period: formatPeriod(observation.observedAt ?? observation.recordedAt, index) }
      current[variableKey] = observation.numericValue ?? undefined
      grouped.set(key, current)
    })
    return Array.from(grouped.entries()).sort(([left], [right]) => left.localeCompare(right)).map(([, value]) => value)
  }, [data, usingRealData])

  const limitedData = useMemo(() => {
    const size = period === '7d' ? 4 : period === '30d' ? 6 : fullData.length
  ...
  const values = temporalData.map(item => Number(item[selectedVariable] ?? 0)).filter(Number.isFinite)
  ...
  const distribution = useMemo(() => {
    ...
    return Array.from({ length: 5 }, (_, index) => {
      ...
      return { range: `${start.toFixed(0)}–${end.toFixed(0)}`, students: values.filter(value => index === 4 ? value >= start && value <= end : value >= start && value < end).length }
    })
  }, [values])
  ...
  const scatterData = limitedData.map(item => ({ x: Number(item[scatterX] ?? 0), y: Number(item[scatterY] ?? 0), z: Number(item[selectedVariable] ?? 1), period: item.period }))
      ...
      <div className="border-b border-slate-200 bg-gradient-to-r from-[#071827] via-[#0B2A40] to-[#0B7491] px-5 py-6 text-white sm:px-7">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            {(['7d', '30d', '90d'] as Period[]).map(option => <button key={option} type="button" onClick={() => setPeriod(option)} className={`rounded-xl px-3 py-2 text-xs font-bold transition ${period === option ? 'bg-white text-[#071827]' : 'border border-white/20 text-white hover:bg-white/10'}`}>{option === '7d' ? 'Curto prazo' : option === '30d' ? '30 dias' : '90 dias'}</button>)}
          </div>
        </div>
