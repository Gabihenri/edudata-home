...
function CycleEvolution({ dimensions }: { dimensions: CycleDimension[] }) {
  const maximum = Math.max(...dimensions.map(item => item.value), 1)
  const available = dimensions.filter(item => item.value > 0)
  const min = available.length ? Math.min(...available.map(item => item.value)) : 0
  const max = available.length ? Math.max(...available.map(item => item.value)) : 0

  return <section className="rounded-2xl border border-slate-200 bg-white p-5">
    <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#0B7491]">Ciclo EDI</p>
        <h3 className="mt-1 text-xl font-bold text-[#071827]">Planejar → Registrar → Evidenciar → Analisar</h3>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Leitura do estágio atual dos registros disponíveis. Nesta fase do MVP, os valores representam volume e cobertura de registros, não uma avaliação de desempenho.</p>
      </div>
      <div className="text-sm text-slate-500">Faixa disponível: <span className="font-semibold text-slate-700">{min}–{max} registros</span></div>
    </div>

    <div className="mt-6 grid gap-4 lg:grid-cols-4">
      {dimensions.map((dimension, index) => {
        const width = `${Math.max((dimension.value / maximum) * 100, dimension.value > 0 ? 8 : 0)}%`
        return <article key={dimension.label} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
          <div className="flex items-center justify-between gap-3">
            <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#071827] text-xs font-bold text-white">{index + 1}</span>
            <span className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500">{dimension.label}</span>
          </div>
          <p className="mt-4 text-3xl font-bold text-[#071827]">{dimension.value}</p>
          <p className="mt-1 min-h-10 text-xs leading-5 text-slate-500">{dimension.value > 0 ? dimension.description : 'Ainda não há base suficiente nesta etapa.'}</p>
          <div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-200">
            <div className="h-full rounded-full bg-[#0B7491] transition-all" style={{ width }} />
          </div>
        </article>
      })}
    </div>

    <div className="mt-5 rounded-xl border border-cyan-100 bg-cyan-50/60 p-4 text-sm leading-6 text-slate-600">
      <span className="font-semibold text-[#075F78]">Como interpretar:</span> uma etapa com menos registros pode significar que ela ainda está sendo incorporada ao fluxo de trabalho. A análise não conclui automaticamente que existe um problema.
    </div>
  </section>
}
...
function CorrelationCard({ item }: { item: AnalyticsCorrelationResult }) {
  return <article className="rounded-2xl border border-slate-200 bg-white p-5">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#0B7491]">{item.method}</p><h4 className="mt-1 font-semibold text-[#071827]">{correlationLabel(item)}</h4></div>
...
export default function EducationalAnalyticsPanel() {
  ...
  const load = useCallback(async (): Promise<void> => {
    ...
    try {
      const response = await fetch('/api/agenda/educational-analytics/operational', { method: 'GET', credentials: 'include', cache: 'no-store', headers: { Accept: 'application/json' } })
  ...
  if (loading) return <section className="rounded-[1.75rem] border border-cyan-200 bg-white p-6 shadow-sm"><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#0B7491]">EIOS · Educational Analytics</p><h2 className="mt-2 text-2xl font-bold text-[#071827]">Processando inteligência educacional</h2><p className="mt-2 text-sm leading-6 text-slate-500">Correlacionando registros, procurando padrões e verificando a cobertura da base disponível.</p></section>
  ...
  return <section className="overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-sm">
    <header className="border-b border-slate-200 bg-[#071827] px-5 py-6 text-white sm:px-7"><p className="text-xs font-bold uppercase tracking-[0.18em] text-cyan-300">EIOS · Educational Analytics</p><div className="mt-2 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-2xl font-bold">Inteligência sobre os registros da Agenda</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">Correlação, padrões e evolução do ciclo educacional a partir de dados operacionais autorizados.</p></div><button type="button" onClick={() => void load()} className="inline-flex min-h-11 items-center justify-center rounded-xl border border-white/20 bg-white/10 px-5 py-3 text-sm font-semibold text-white transition hover:bg-white/15">Reprocessar análise</button></div></header>
    <div className="space-y-7 p-5 sm:p-7">
      <section className="rounded-2xl border border-cyan-100 bg-cyan-50/50 p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#0B7491]">Base da análise</p><h3 className="mt-1 text-xl font-bold text-[#071827]">{coverageLabel}: {availableDimensions.length} de {dimensions.length} dimensões com dados</h3><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Ausência de registros não é interpretada como baixo desempenho. Os resultados devem ser lidos de acordo com a cobertura e as limitações da base.</p></div><div className="rounded-xl border border-cyan-200 bg-white px-4 py-3 text-left lg:text-right"><p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500">Cobertura</p><p className="mt-1 text-2xl font-bold text-[#075F78]">{formatPercentage(coverageRate)}</p><p className="mt-1 text-xs text-slate-500">Atualizado em {formatDate(data.snapshot?.generatedAt)}</p></div></div><p className="mt-4 text-xs leading-5 text-slate-500">Fonte atual: Agenda Inteligente EDI. Outras fontes autorizadas poderão ser incorporadas progressivamente.</p></section>

      <CycleEvolution dimensions={dimensions} />

      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Observações" value={String(analytics.observations.length)} description="Registros convertidos para o contrato analítico." /><Metric label="Correlações" value={String(analytics.correlations.length)} description="Associações estatísticas avaliadas pelos motores EIOS." /><Metric label="Padrões" value={String(analytics.patterns.length)} description="Tendências e recorrências detectadas." /><Metric label="Pontos para revisão" value={String(analytics.anomalies.length)} description="Sinais que exigem interpretação humana." /></div>

      {correlations.length > 0 ? <div><div className="mb-4"><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#0B7491]">Correlação</p><h3 className="mt-1 text-xl font-bold text-[#071827]">Relações com maior magnitude</h3></div><div className="grid gap-4 lg:grid-cols-2">{correlations.map(item => <CorrelationCard key={item.id} item={item} />)}</div></div> : null}
