...
const ALLOWED_INTELLIGENCE_ROLES = new Set<IntelligenceRole>([
  'professor',
  'coordenador',
  'diretor',
  'gestor',
...
function normalizeRoleValue(value: unknown): IntelligenceRole | null {
  ...
  return ALLOWED_INTELLIGENCE_ROLES.has(normalized as IntelligenceRole)
    ? (normalized as IntelligenceRole)
    : null
}
...
function getIntelligenceBackendUrl(): string {
  const baseUrl =
    process.env.EDI_BACKEND_URL ??
    process.env.BACKEND_API_URL ??
    process.env.NEXT_PUBLIC_API_URL

  if (!baseUrl?.trim()) {
    throw new Error('A URL do backend EIOS não está configurada.')
  }

  return `${baseUrl
    .trim()
    .replace(/\/+$/, '')
    .replace(/\/api\/v1$/, '')}/api/v1/intelligence/agenda`
}
...
function isRetryableNetworkError(error: unknown): boolean {
  ...
  if (
    ...
    ['network', 'socket', 'connection', 'timeout', 'timed out'].some(value =>
...
function inferContext(
  ...
  const organizationId = allRecords
    .map(record => normalizeOptionalText(record.organization_id))
    .find(Boolean) ?? null
  const schoolId = allRecords
    .map(record => normalizeOptionalText(record.school_id))
    .find(Boolean) ?? null
  ...
  return {
    ...
    role,
    metadata: {
      source: 'next-agenda-intelligence-route',
      scope: 'authenticated-user',
      rls_applied: true,
      snapshot_source: 'operational-snapshot-service',
      learning_source: 'agenda-learning-events',
      contract_version: 'agenda-operational-v2',
      role_source: 'authenticated-user-metadata',
    },
  }
}
...
async function performBackendRequest({
  ...
  attempt: number
}): Promise<IntelligenceBackendResponse> {
  ...
  try {
    const response = await fetch(backendUrl, {
      ...
      headers: {
        ...
        'X-EDI-Contract-Version': 'agenda-operational-v2',
        'X-EDI-Request-Attempt': String(attempt),
      },
    ...
    const parsedBody: IntelligenceBackendResponse = isRecord(responseBody)
      ? (responseBody as IntelligenceBackendResponse)
      : {}

    if (!response.ok || parsedBody.success === false) {
      throw new Error(
        normalizeOptionalText(parsedBody.detail) ??
          normalizeOptionalText(parsedBody.message) ??
...
async function callIntelligenceBackend(
  ...
  for (let attempt = 1; attempt <= MAX_BACKEND_ATTEMPTS; attempt += 1) {
    try {
      ...
      console.error('[EDI_INTELLIGENCE_BACKEND_ATTEMPT_ERROR]', {
        attempt,
        maximumAttempts: MAX_BACKEND_ATTEMPTS,
        backendUrl,
        retryable,
  ...
  if (lastError instanceof Error && lastError.name === 'AbortError') {
    throw new Error('O EDI Intelligence Engine excedeu o tempo máximo de resposta.')
  }
  ...
  throw lastError instanceof Error
    ? lastError
    : new Error('Não foi possível acessar o backend EIOS.')
}

function buildSuccessResponse(
  backendResponse: IntelligenceBackendResponse,
): NextResponse {
  const backendData = isRecord(backendResponse.data)
    ? backendResponse.data
    : {}
  const engine = isRecord(backendData.engine)
    ? backendData.engine
    : {}
  ...
  return NextResponse.json(
    {
      success: true,
      generated_at: normalizeOptionalText(backendData.generated_at),
      module: normalizeOptionalText(backendData.module) ?? 'agenda',
      contract_version:
        normalizeOptionalText(backendData.contract_version) ??
        'agenda-operational-v2',
      engine,
    },
    {
      status: 200,
...
function getErrorStatus(error: unknown): number {
  ...
  if (message.includes('não autenticado') || message.includes('unauthorized')) {
    return 401
  }
  if (message.includes('não configurada') || message.includes('excedeu')) {
    return 503
  }
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const [snapshot, learningContext] = await Promise.all([
      loadAgendaOperationalSnapshot({
        client,
        userId: user.id,
      }),
    ...
    if (isAccessDeniedError(error)) {
      return serializeAccessDeniedError(error)
    }
    ...
    return NextResponse.json(
      {
        ...
        error:
          status >= 500
            ? 'Não foi possível gerar as orientações da Agenda neste momento.'
