...
from app.routers.professor_digital_intelligence import (
    router as professor_digital_intelligence_router,
)
...
from app.services.capabilities.bootstrap import capability_bootstrap


@asynccontextmanager
async def lifespan(_app: FastAPI) -> AsyncIterator[None]:
    bootstrap_report = capability_bootstrap.initialize()

    if not bootstrap_report.success:
        raise RuntimeError(
            "A Educational Capability Platform não foi inicializada de forma consistente."
        )
    ...
    print(
        "[EDUDATA_API_STARTUP_COMPLETED]",
        {
            ...
            "registry_total": bootstrap_report.registry_total,
            "registered_handlers": len(bootstrap_report.registered_handlers),
        },
    )

    yield

    print("[EDUDATA_API_SHUTDOWN_COMPLETED]", {"ecp_initialized": True})
...
app.include_router(teacher_intelligence_router)

# ==========================================================
# PROFESSOR DIGITAL
# ==========================================================
app.include_router(professor_digital_intelligence_router)
...
def health() -> dict[str, str | bool | int]:
    ecp_status = capability_bootstrap.status()
    return {
        "status": "healthy",
        "service": "EduData IA API",
        "version": "1.0.0",
        "ecp_initialized": bool(ecp_status.get("initialized", False)),
        "registered_capabilities": int(ecp_status.get("registry_total", 0)),
    }
