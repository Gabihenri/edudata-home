...
-- Manifesto central da bateria R01–R12; não executa SQL externo.
...
SELECT
  ...
  CASE WHEN COUNT(*)=12 AND COUNT(DISTINCT harness)=6
       THEN 'PASS_REGRESSION_REGISTRY_COMPLETE'
       ELSE 'FAIL_REGRESSION_REGISTRY_COMPLETE' END AS assertion
FROM test_registry;

-- O runner deve executar os seis harnesses referenciados e registrar evidência
-- de cada caso. Esta suite, isoladamente, é um manifesto e não declara PASS
-- da execução dos harnesses.

-- Critério de promoção:
-- qualquer falha crítica R01/R03/R04/R05/R06/R07/R11/R12 bloqueia a promoção.
