...
CREATE TABLE teacher_load_rows (
  ...
  CHECK (matching_status NOT IN ('resolved') OR (teacher_profile_id IS NOT NULL AND matching_method IS NOT NULL)),
  CHECK (matching_status <> 'resolved' OR validation_status = 'validated')
);
CREATE TABLE homologation_permissions (
  ...
  organization_id uuid NOT NULL,
  school_id uuid NOT NULL,
  PRIMARY KEY (user_id, permission_code, organization_id, school_id),
  FOREIGN KEY (organization_id, school_id, user_id) REFERENCES organization_members(organization_id, school_id, user_id)
);
...
CREATE TABLE academic_teacher_identity_links (
  ...
  FOREIGN KEY (school_id, organization_id) REFERENCES schools(id, organization_id),
  FOREIGN KEY (teacher_profile_id, organization_id, school_id) REFERENCES teacher_profiles(id, organization_id, school_id),
  FOREIGN KEY (auth_user_id) REFERENCES auth_users(id),
  FOREIGN KEY (organization_id, school_id, auth_user_id) REFERENCES organization_members(organization_id, school_id, user_id)
);
ALTER TABLE academic_teacher_identity_links ADD CONSTRAINT identity_link_no_overlap
EXCLUDE USING gist (
...
CREATE FUNCTION forbid_weak_matching() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF NEW.matching_method IN ('name','email','name_only','email_only','name_or_email','fuzzy') THEN
    RAISE EXCEPTION 'weak matching is forbidden';
  END IF;
  RETURN NEW;
END $$;
CREATE TRIGGER trg_forbid_weak_matching BEFORE INSERT OR UPDATE ON teacher_load_rows
FOR EACH ROW EXECUTE FUNCTION forbid_weak_matching();

CREATE FUNCTION validate_active_identity_link() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE a boolean; m boolean; t text; p boolean;
BEGIN
  SELECT active INTO a FROM auth_users WHERE id = NEW.auth_user_id;
  SELECT active INTO m FROM organization_members WHERE organization_id=NEW.organization_id AND school_id=NEW.school_id AND user_id=NEW.auth_user_id;
  SELECT status INTO t FROM teacher_profiles WHERE id=NEW.teacher_profile_id;
  SELECT EXISTS (SELECT 1 FROM homologation_permissions WHERE user_id=NEW.verified_by AND permission_code='escala.manage_teacher_identity_links' AND organization_id=NEW.organization_id AND school_id=NEW.school_id) INTO p;
  IF NEW.status='active' AND NOT COALESCE(a,false) THEN RAISE EXCEPTION 'inactive auth'; END IF;
  IF NEW.status='active' AND NOT COALESCE(m,false) THEN RAISE EXCEPTION 'inactive membership'; END IF;
  IF NEW.status='active' AND t IS DISTINCT FROM 'active' THEN RAISE EXCEPTION 'inactive teacher'; END IF;
  IF NEW.status='active' AND NOT COALESCE(p,false) THEN RAISE EXCEPTION 'missing homologation permission'; END IF;
  RETURN NEW;
END $$;
CREATE TRIGGER trg_validate_active_identity_link BEFORE INSERT OR UPDATE ON academic_teacher_identity_links
FOR EACH ROW EXECUTE FUNCTION validate_active_identity_link();

CREATE FUNCTION assert_true(label text, condition boolean) RETURNS void LANGUAGE plpgsql AS $$
BEGIN IF NOT COALESCE(condition,false) THEN RAISE EXCEPTION 'ASSERTION FAILED: %', label; END IF; END $$;
...
('00000000-0000-0000-0000-000000000001','00000000-0000-0000-0000-000000000011','10000000-0000-0000-0000-000000000004',true);
...
INSERT INTO homologation_permissions VALUES
('10000000-0000-0000-0000-000000000001','escala.manage_teacher_identity_links','00000000-0000-0000-0000-000000000001','00000000-0000-0000-0000-000000000011'),
('10000000-0000-0000-0000-000000000002','escala.manage_teacher_identity_links','00000000-0000-0000-0000-000000000001','00000000-0000-0000-0000-000000000012');

-- DOC-01: replay idempotente do mesmo lote.
...
DO $$ BEGIN
  BEGIN INSERT INTO teacher_load_batches VALUES ('30000000-0000-0000-0000-000000000002','00000000-0000-0000-0000-000000000001','00000000-0000-0000-0000-000000000011','sed_grade','HASH-001','2026-09'); RAISE EXCEPTION 'DOC-01 failed';
  EXCEPTION WHEN unique_violation THEN NULL; END;
END $$;
PERFORM assert_true('DOC-01', (SELECT count(*)=1 FROM teacher_load_batches WHERE source_hash='HASH-001'));

-- DOC-02: identificador institucional determina resolução.
...
PERFORM assert_true('DOC-02', (SELECT matching_status='resolved' AND teacher_profile_id='20000000-0000-0000-0000-000000000001'::uuid FROM teacher_load_rows WHERE id='31000000-0000-0000-0000-000000000001'));

-- DOC-03: nome isolado permanece unresolved.
...
PERFORM assert_true('DOC-03', (SELECT matching_status='unresolved' AND teacher_profile_id IS NULL FROM teacher_load_rows WHERE id='31000000-0000-0000-0000-000000000003'));

-- DOC-04: email isolado permanece unresolved.
...
PERFORM assert_true('DOC-04', (SELECT matching_status='unresolved' AND teacher_profile_id IS NULL FROM teacher_load_rows WHERE id='31000000-0000-0000-0000-000000000004'));

-- DOC-05: múltiplos candidatos ficam ambiguous.
...
PERFORM assert_true('DOC-05', (SELECT matching_status='ambiguous' AND teacher_profile_id IS NULL FROM teacher_load_rows WHERE id='31000000-0000-0000-0000-000000000005'));

-- DOC-06: organização incompatível não pode criar identidade acadêmica.
...
-- DOC-07: escola incompatível também é bloqueada pela integridade de escopo.
...
-- DOC-08: Auth sem professor institucional não produz link ativo.
DO $$ BEGIN BEGIN INSERT INTO academic_teacher_identity_links VALUES ('40000000-0000-0000-0000-000000000008','00000000-0000-0000-0000-000000000001','00000000-0000-0000-0000-000000000011','20000000-0000-0000-0000-000000000001','10000000-0000-0000-0000-000000000004','active','2026-01-01',NULL,'test', '10000000-0000-0000-0000-000000000004',now(),'{}'); RAISE EXCEPTION 'DOC-08 failed: expected permission block'; EXCEPTION WHEN raise_exception THEN IF SQLERRM NOT LIKE 'missing homologation permission' THEN RAISE; END IF; END; END $$;

-- DOC-09: professor acadêmico pode existir sem Auth.
PERFORM assert_true('DOC-09', (SELECT status='active' FROM teacher_profiles WHERE id='20000000-0000-0000-0000-000000000001'));

-- DOC-10: homologação válida gera link ativo.
...
PERFORM assert_true('DOC-10', (SELECT status='active' AND verified_by IS NOT NULL AND verified_at IS NOT NULL FROM academic_teacher_identity_links WHERE id='40000000-0000-0000-0000-000000000010'));

-- DOC-11: homologador sem permissão é bloqueado.
...
-- DOC-12: intervalos incompatíveis não podem coexistir.
...
-- DOC-13: multi-school legítima usa contexto próprio e não exige unicidade global.
...
PERFORM assert_true('DOC-13', (SELECT count(*)=1 FROM academic_teacher_identity_links WHERE auth_user_id='10000000-0000-0000-0000-000000000002'));

-- DOC-14: fuzzy/name/email nunca são método ativo de matching.
...
-- DOC-15: revogação preserva histórico.
...
PERFORM assert_true('DOC-15', (SELECT status='revoked' AND valid_until='2026-08-31' FROM academic_teacher_identity_links WHERE id='40000000-0000-0000-0000-000000000010'));

-- DOC-16: correção de fonte cria nova evidência/batch, sem sobrescrever raw antigo.
...
PERFORM assert_true('DOC-16', (SELECT count(*)=2 FROM teacher_load_rows WHERE source_record_id='SRC-001'));

-- DOC-17: raw bruto é imutável por contrato de harness.
PERFORM assert_true('DOC-17', (SELECT raw_payload='{"name":"Professor A","id":"T-001"}'::jsonb FROM teacher_load_rows WHERE id='31000000-0000-0000-0000-000000000001'));

-- DOC-18: homologação produz evento de auditoria central.
INSERT INTO identity_audit_logs(event_type,actor_user_id,organization_id,school_id,subject_id,provenance)
VALUES ('teacher_identity_homologated','10000000-0000-0000-0000-000000000001','00000000-0000-0000-0000-000000000001','00000000-0000-0000-0000-000000000011','20000000-0000-0000-0000-000000000001','{"source":"teacher_load"}');
PERFORM assert_true('DOC-18', (SELECT count(*)=1 FROM identity_audit_logs WHERE event_type='teacher_identity_homologated'));

-- DOC-19: reprodução histórica por data encontra o vínculo vigente na ocorrência.
...
PERFORM assert_true('DOC-19', (SELECT count(*)=1 FROM academic_teacher_identity_links WHERE teacher_profile_id='20000000-0000-0000-0000-000000000001' AND DATE '2027-06-15' BETWEEN valid_from AND valid_until AND status='pending'));

-- DOC-20: rejeitado/unresolved nunca pode aparecer como identidade acadêmica homologada.
PERFORM assert_true('DOC-20', (SELECT count(*)=0 FROM teacher_load_rows r WHERE r.matching_status IN ('rejected','unresolved','ambiguous') AND r.teacher_profile_id IS NOT NULL));

RAISE NOTICE 'DOC-01..DOC-20: PASS';
ROLLBACK;
