...
-- S01: maximum evidence candidate.
...
-- Materialize v1 results from deterministic calculation, including tie-break rules.
WITH calculated AS (
  SELECT
  ...
  SELECT *,
    row_number() OVER (
      ORDER BY score DESC,
               component_match DESC,
               area_match DESC,
               continuity DESC,
               distribution_balance DESC,
               teacher_id ASC
    ) AS rank_position
  FROM calculated
)
...
-- Excluded candidate has no operational score.
INSERT INTO escala_score_test.scores (
  run_id, candidate_id, score, ranking_status, rank_position,
  component_contribution, area_contribution, availability_contribution,
  continuity_contribution, distribution_contribution, proximity_contribution,
...
-- Materialize v2 for the same eligible population with a genuinely changed rule set.
WITH calculated AS (
  SELECT
    c.id AS candidate_id,
    c.teacher_id,
...
-- GATE-01: every supported rule-set totals exactly 100.
DO $$
BEGIN
  IF EXISTS (
...
-- GATE-02: operational scores remain in 0-100.
DO $$
BEGIN
...
-- GATE-03: HARD eligibility barrier is absolute.
DO $$
BEGIN
  IF EXISTS (
...
-- GATE-04: maximum evidence reconstructs exactly 100 in both runs.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM escala_score_test.scores
    WHERE candidate_id = '10000000-0000-0000-0000-000000000001'
      AND score <> component_contribution + area_contribution + availability_contribution +
...
-- GATE-05: component match changes the score when other criteria are equal.
DO $$
BEGIN
  IF escala_score_test.calculate_score('10000000-0000-0000-0000-000000000002', 'score-v1')
     <= escala_score_test.calculate_score('10000000-0000-0000-0000-000000000003', 'score-v1') THEN
    RAISE EXCEPTION 'GATE-05 failed: component match did not affect score';
  END IF;
END $$;

-- GATE-06: unavailable proximity evidence is explicit and never advantageous.
DO $$
DECLARE evidence_status text;
BEGIN
  SELECT proximity_evidence_status INTO evidence_status
  ...
  IF escala_score_test.calculate_score('10000000-0000-0000-0000-000000000007', 'score-v1')
     >= escala_score_test.calculate_score('10000000-0000-0000-0000-000000000001', 'score-v1') THEN
    RAISE EXCEPTION 'GATE-06 failed: missing proximity evidence created an advantage';
  END IF;
END $$;

-- GATE-07: complete ties use only homologated academic technical id as final tie-break.
DO $$
...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION 'GATE-07 failed: tie-break identifier is not homologated';
  END IF;
  ...
  IF first_teacher <> '50000000-0000-0000-0000-000000000005' THEN
    RAISE EXCEPTION 'GATE-07 failed: deterministic tie-break not reproducible';
  END IF;
END $$;

-- GATE-08: run provenance is complete.
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    WHERE id = '90000000-0000-0000-0000-000000000002'
      AND rule_set_version = 'score-v2'
      AND snapshot_reference = 'snapshot-2026-09-10-v2'
  ) THEN
    RAISE EXCEPTION 'GATE-08 failed: run provenance missing';
  END IF;
END $$;

-- GATE-09: ranking is actually produced by deterministic ORDER BY over calculated scores.
DO $$
DECLARE calculated_ids uuid[];
DECLARE stored_ids uuid[];
BEGIN
  ...
  SELECT array_agg(candidate_id ORDER BY score DESC, component_match DESC, area_match DESC,
  ...
  IF stored_ids <> calculated_ids THEN
    RAISE EXCEPTION 'GATE-09 failed: materialized ranking differs from deterministic calculation';
  END IF;
END $$;

-- GATE-10: rule-set v2 changes the score of a controlled candidate.
DO $$
BEGIN
  IF escala_score_test.calculate_score('10000000-0000-0000-0000-000000000008', 'score-v1')
     = escala_score_test.calculate_score('10000000-0000-0000-0000-000000000008', 'score-v2') THEN
    RAISE EXCEPTION 'GATE-10 failed: changed rule-set produced identical controlled result';
  END IF;
END $$;

-- GATE-11: v1 historical result remains exactly unchanged after v2 execution.
DO $$
DECLARE before_score numeric;
DECLARE after_score numeric;
BEGIN
  SELECT score INTO before_score
  FROM escala_score_test.scores
  WHERE run_id = '90000000-0000-0000-0000-000000000001'
    AND candidate_id = '10000000-0000-0000-0000-000000000008';

  SELECT score INTO after_score
  FROM escala_score_test.scores
  WHERE run_id = '90000000-0000-0000-0000-000000000008';

  IF before_score IS NULL OR before_score <> 85 THEN
    RAISE EXCEPTION 'GATE-11 failed: v1 historical result was not preserved';
  END IF;
END $$;

-- GATE-12: score breakdown contains criterion evidence and reason code for unavailable proximity.
DO $$
DECLARE proximity_reason text;
BEGIN
  SELECT score_breakdown #>> '{criteria,5,reason_code}'
    INTO proximity_reason
  FROM escala_score_test.scores
  WHERE run_id = '90000000-0000-0000-0000-000000000001'
    AND candidate_id = '10000000-0000-0000-0000-000000000007';
  IF proximity_reason <> 'PROXIMITY_NOT_AVAILABLE' THEN
    RAISE EXCEPTION 'GATE-12 failed: score explanation lacks proximity reason code';
  END IF;
END $$;

-- GATE-13: one run may contain only one result per candidate, while different runs preserve history.
DO $$
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION 'GATE-13 failed: duplicate candidate result within a run';
  END IF;
  IF (SELECT count(*) FROM escala_score_test.scores WHERE candidate_id = '10000000-0000-0000-0000-000000000008') <> 2 THEN
    RAISE EXCEPTION 'GATE-13 failed: historical results were overwritten or lost';
  END IF;
END $$;
