...
create table if not exists public.agenda_tasks (
  ...
  constraint agenda_tasks_completion_check check (
    (status <> 'completed') or (completed_at is not null and completed_by is not null)
  ),
  constraint agenda_tasks_cancellation_check check (
...
create index if not exists agenda_tasks_user_status_idx
  on public.agenda_tasks(user_id, status, due_at)
...
begin
  ...
  if v_task.status in ('completed','cancelled') and p_new_status <> v_task.status then
  ...
  insert into public.agenda_task_status_history (
    ...
    v_task.id, v_task.user_id, (select status from public.agenda_tasks where id = p_task_id for share), p_new_status, v_actor, p_reason
  );
...
create policy "agenda_tasks_owner_select"
  on public.agenda_tasks for select
  using (auth.uid() = user_id);
create policy "agenda_tasks_owner_insert"
  on public.agenda_tasks for insert
  with check (auth.uid() = user_id);
create policy "agenda_tasks_owner_update"
  on public.agenda_tasks for update
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

create policy "agenda_task_status_history_owner_select"
  on public.agenda_task_status_history for select
  using (auth.uid() = user_id);
create policy "agenda_task_status_history_owner_insert"
  on public.agenda_task_status_history for insert
