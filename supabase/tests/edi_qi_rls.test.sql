...
-- Synthetic authenticated users. The transaction is rolled back at the end.
-- These UUIDs are test-only identities and do not need real Auth sessions.
...
-- Direct authorization functions must deny a peer from acting on another user's record.
select ok(not public.can_view_agenda_record('00000000-0000-0000-0000-0000000000a1'::uuid), 'Peer B cannot view A record without an authorized relationship');
select ok(not public.can_update_agenda_record('00000000-0000-0000-0000-0000000000a1'::uuid), 'Peer B cannot update A record without an authorized relationship');
